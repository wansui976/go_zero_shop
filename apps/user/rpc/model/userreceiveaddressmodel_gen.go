...
func newUserReceiveAddressModel(conn sqlx.SqlConn, c cache.CacheConf, opts ...cache.Option) *defaultUserReceiveAddressModel {
...
func (m *defaultUserReceiveAddressModel) FindOne(ctx context.Context, id int64) (*UserReceiveAddress, error) {
	...
	err := m.QueryRowCtx(ctx, &resp, userReceiveAddressIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultUserReceiveAddressModel) Insert(ctx context.Context, data *UserReceiveAddress) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Uid, data.Name, data.Phone, data.IsDefault, data.Province, data.City, data.Region, data.DetailAddress, data.IsDelete)
	}, userReceiveAddressIdKey)
	return ret, err
}
...
func (m *defaultUserReceiveAddressModel) Update(ctx context.Context, data *UserReceiveAddress) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Uid, data.Name, data.Phone, data.IsDefault, data.Province, data.City, data.Region, data.DetailAddress, data.IsDelete, data.Id)
	}, userReceiveAddressIdKey)
	return err
}

func (m *defaultUserReceiveAddressModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheUserReceiveAddressIdPrefix, primary)
}

func (m *defaultUserReceiveAddressModel) queryPrimary(ctx context.Context, conn sqlx.SqlConn, v, primary any) error {
	query := fmt.Sprintf("select %s from %s where `id` = ? limit 1", userReceiveAddressRows, m.table)
